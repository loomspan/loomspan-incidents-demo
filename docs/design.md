...
The first slice includes four component controls, four presets, persistent
incidents, asynchronous model investigation, three focused specialists, saved
evidence, supported repair proposals, verified resolution and historical runs.
...
- `ProbeSkills`: three annotation-defined Java probe leaves plus a read of the
...
`OPEN`. A repair moves it to `MITIGATED`, not resolved. Verification either moves
it to `RESOLVED` or back to `OPEN`. Verification also supports manual recovery:
...
- One storefront and one instance per service; no tenants, real hosts or external
  monitoring integration.
- No random failure generation, real clock progression, cache, load balancing,
...
- The first version uses fixed latency values and qualitative observations rather
  than fabricated historical metric charts. Deployment history is simulated.
...
1. Two checkout instances and capacity: demonstrate redundancy loss versus an outage.
2. Cache degradation with database load: trace a cascading performance problem.
3. Additional path-specific failures such as DNS and region-specific connectivity.
4. Role-constrained remediation and an explicitly enabled automatic repair policy.
5. Controlled snapshot refresh during investigation, if the framework's supported
