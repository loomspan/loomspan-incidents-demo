...
public final class Simulation {
    ...
    public static DataLoad dataLoad(World w) {
        ...
        int admitted=w.badDeploy()||!w.linkAllowed()||!w.databaseRunning()?0:Math.min(w.demandRps(),online*INSTANCE_CAPACITY);
    ...
    public static World repair(World w,String action) {
        boolean a=w.checkoutRunning(),db=w.databaseRunning(),link=w.linkAllowed(),bad=w.badDeploy();Boolean b=w.checkoutBRunning(),cache=w.cacheRunning();Integer hit=w.cacheHitPercent();
        switch(action) {
        ...
        return new World(w.revision()+1,a,db,link,bad,b,w.demandRps(),cache,hit);
    }
}
